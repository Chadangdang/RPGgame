...
class GameMainRenderMixin:
    ...
    def _model_row_gap(self, prev_idx: int | None, idx: int) -> int:
        ...
        if (prev_idx == 2 and self._model_is_personality_subrow(idx)) or (
        ...
        if self.game_screen == -1:      # Start screen
            ...
            def draw_column(is_left: bool) -> None:
                ...
                for idx, rect in self._model_row_rects(is_left):
                    ...
                    text_font = self.font_sm if not self._model_is_personality_subrow(idx) else self.font_s
                    text_surface = text_font.render(model_names[idx], False, (0, 0, 0))
                    if self._model_is_personality_subrow(idx):
            ...
            AI_types = ('Player Input', 'Baseline', 'Personality Cores', 'Aggressive', 'Strategic', 'Survival', 'Random', 'Disable AI')
