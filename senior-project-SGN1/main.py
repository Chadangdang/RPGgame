...
class GameMain:
    ...
    def __init__(self) -> None:
        ...
        self._log_line_h = 22
        self._log_scroll_step = 1
        self._log_page_step = 8
    ...
    def _calc_log_geometry(self):
        ...
        if max_scroll == 0:
            ...
            t = (self.log_scroll / max_scroll) if max_scroll else 0
    ...
    # ---------- tile rect helper (for perfect highlight alignment) ----------
    def _grid_to_rect(self, gx: int, gy: int) -> pygame.Rect:
        """
        Return the exact pixel rect for the grid cell (gx, gy).
        Prefer the box's own rect if available; otherwise compute from field origin.
        """
        try:
            box = self.field.boxes[gx][gy]
            if hasattr(box, "rect") and box.rect is not None:
                return box.rect.copy()
        except Exception:
            pass

        # Fallback origins the Field might expose
        if hasattr(self.field, "pos"):
            ox, oy = self.field.pos
        elif hasattr(self.field, "position"):
            ox, oy = self.field.position
        elif hasattr(self.field, "origin"):
            ox, oy = self.field.origin
        else:
            ox, oy = BOARD_POS_X, BOARD_POS_Y

        w = self.field.boxes_width
        h = self.field.boxes_height
        return pygame.Rect(ox + gx * w, oy + gy * h, w, h)
    ...
    def render(self) -> None:
        if self.game_screen == 0:
            ...
            p1_text = self.font_l.render("Player 1", False, (0, 0, 0))
            text_rect = p1_text.get_rect(midtop=(WIDTH // 4, 40))
            self.screen.blit(p1_text, text_rect)

            p2_text = self.font_l.render("Player 2", False, (0, 0, 0))
            ...
            text = self.font_sm.render('Press X or ENTER to start', False, (0, 0, 0))
            ...
            if self.map_number == len(self.map_list):
                map_text = self.font_s.render("Map : Random", False, (0, 0, 0))
            ...
            # === Finished-Action Highlight (exact tile, no drift) ===
            # Draw AFTER the board so the border shows on top.
            for chara in Character.team1_list + Character.team2_list:
                if getattr(chara, "acted", False):
                    rect = self._grid_to_rect(chara.grid[0], chara.grid[1])
                    color = (255, 215, 0) if chara in Character.team1_list else (0, 200, 255)
            ...
            text = self.font_s.render('Have units stand', False, (0, 0, 0))
            text_rect = text.get_rect(topleft=(55, 480))
            self.screen.blit(text, text_rect)
            text = self.font_s.render('in objective area', False, (0, 0, 0))
            text_rect = text.get_rect(topleft=(55, 505))
            self.screen.blit(text, text_rect)
            text = self.font_s.render('more than enemy.', False, (0, 0, 0))
            ...
            if self.game_state == 'win':
                pygame.draw.rect(self.screen, WHITE, pygame.Rect(WIDTH // 2 - 300, HEIGHT // 2 - 90, 600, 180))
                pygame.draw.rect(self.screen, BLACK, pygame.Rect(WIDTH // 2 - 300, HEIGHT // 2 - 90, 600, 180), 4)
                round_text = self.font_l.render("P1 Win!!", False, (0, 0, 0))
                text_rect = round_text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
                self.screen.blit(round_text, text_rect)
            ...
            if self.game_state == 'lose':
                pygame.draw.rect(self.screen, WHITE, pygame.Rect(WIDTH // 2 - 300, HEIGHT // 2 - 90, 600, 180))
                pygame.draw.rect(self.screen, BLACK, pygame.Rect(WIDTH // 2 - 300, HEIGHT // 2 - 90, 600, 180), 4)
                round_text = self.font_l.render("P2 Win!!", False, (0, 0, 0))
                text_rect = round_text.get_rect(center=(WIDTH // 2, HEIGHT // 2))
                self.screen.blit(round_text, text_rect)
